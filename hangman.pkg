class Hangman():
    ...
    def play(self):
        ...
        while True:
            ...
            if user_input not in Hangman.game_progress: # 플레이어가 입력한 인풋이 유효한 값인지 아닌지.
                print('다시입력')# 아니면, 제대로 입력하라는 메세지 출력하고
                
                break# break or continue 중 하나를 사용하여 계속 입력 받음.
            # Check if the letter is not already guessed
            if user_input in Hangman.game_progress:
                print('이미 입력됨')# 이미 입력받았던 알파벳이면, 이미 입력했다 메시지 출력
                break  # break or continue 중 하나를 사용하여 계속 입력

            # when the user_input is a character in word_to_guess
            if user_input in Hangman.word_to_guess:
                indexes = index(user_input) # 추측한 글자의 인덱스 받기
                Hangman.update_progress(user_input, indexes) # 입력받은 글자로 현재 게임 상태 업데이트

                # If there is no letter to find in the word
                if Hangman.game_progress == self.word_to_guess: # 더이상 맞출 글자가 없으면
                    print('\n------Yay! You win!')
